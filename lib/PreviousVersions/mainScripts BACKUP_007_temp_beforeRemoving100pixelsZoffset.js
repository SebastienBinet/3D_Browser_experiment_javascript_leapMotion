...
//    console.groupEnd();
    ...
    G_bookmarkTree.ThreeJsObjectInScene = createThreeJsObject(G_bookmarkTree); //"desktop");
...
//  }
  ...
  function find_nearest2D_whenNoObjectSelected(){
    ...
    if (!G_grabbedInfo.grabbedElement) {
      ...
      if (G_pinchPoint.position.z < -100) {
 ...
 if( 
   ...
   G_pinchStrength <= G_pinchStrengthCutoff
    ...
    if (G_grabbedInfo.grabbedElement) {
      ...
      G_grabbedInfo.grabbedElement = null;
      G_grabbedInfo.parentIsGrabbed = false;
      G_grabbedInfo.childIsGrabbed = true;
    }
...
function moveSelectedSelectable(){
  ...
  if (zGrabbed !== undefined && zCurrent !== undefined && G_grabbedInfo !== undefined && G_grabbedInfo.grabbedElement !== undefined ) {
    var scaleCurr = computeScaleI_m1p1(zGrabbed, zCurrent);
    G_grabbedInfo.grabbedElement.ThreeJsObjectInScene.scale = scaleCurr;
  }
  // zzz
}
////function moveSelectedSelectable(){
...
function computeScaleI_m1p1(zGrabbed, zCurrent) {
    var scaleI = -1;
...
function performTestVectors() {
    ...
    for (i=0; i< qtyVectors; i++) {
        var in_zGrabbed   = testVectorDragI[i][   in_zGrabbed_idx], 
            ...
            resultComputeScaleI     = computeScaleI_m1p1(in_zGrabbed, in_zCurrent),
